...
//40.418889 , -3.691944
  ...
  geojsonFeature.forEach(function(vert){
    ...
    latc = (parseFloat(lat[0].slice(0,-1))) + (parseFloat(lat[1].slice(0,-1))/60) + (parseFloat(lat[2].slice(0,-2))/3600);
    var signe = lon[3] == 'W' ? '-' : '+';

    lonc = signe + ((parseFloat(lon[0].slice(0,-1))) + (parseFloat(lon[1].slice(0,-1))/60) + (parseFloat(lon[2].slice(0,-2))/3600));
    ...
    //Addd marker at festival locations
